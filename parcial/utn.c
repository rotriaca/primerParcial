...
int getStringAlpha(char mensaje[],char input[])
{
    char aux[256];
    getString(mensaje,aux);
...
int getValidStringAlpha(char requestMessage[],char errorMessage[], char errorMessageLenght[],char input[], int maxLenght,int attemps)
{
    int i;
    int retorno=-1;
    char buffer[1024];
    ...
    for(i=0;i<attemps;i++)
    {
        if (!getStringAlpha(requestMessage,buffer))
        {
            printf ("%s",errorMessage);
            continue;
        }
